...
class MEOCloudCaja(Caja.InfoProvider, Caja.MenuProvider,
    ...
    def touch(self, path):
        item = self.caja_items.get(path, None)
        if item is not None:
    ...
    def update_file_info_full(self, provider, handle, closure, item):
        ...
        saved_item = self.caja_items.get(path)
        if saved_item is None:
            self.caja_items[path] = item
            item.connect('changed', self.changed_cb)
        elif saved_item != item:
            del self.caja_items[path]
            self.caja_items[path] = item
            item.connect('changed', self.changed_cb)
